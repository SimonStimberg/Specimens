...
void PolySynth::addVoice(float pitch) {
    ...
    // std::vector <float> prevPitches;

    // for( size_t i=0; i<voices.size(); ++i ){
    //     prevPitches.push_back(voices[i].meter_pitch());
    // }

    // prevPitches.push_back(pitch);
    ...
    voiceAmps[thisNum].set(0.0);
    // voiceAmps[thisNum].enableSmoothing(50.0f); // 50ms smoothing
    ...
    // v->setup( *this, thisNum, pitch);

    // v->out("L") >> chorus.ch(0);
    // v->out("R") >> chorus.ch(1);

    // 0.0 >> v->in_pan();
    // voiceLevels[thisNum].set(0.5);
    // voiceFilterCutoffs[thisNum].set(0.0);
