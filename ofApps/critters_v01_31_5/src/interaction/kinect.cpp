...
void kinectHandler::setup(vector<float> sizeFactors) {
    ...
    previousTouchPoints.resize(kinect.getWidth());      
...
void kinectHandler::computePoints() {
    ...
    previousTouchPoints = touchPoints;  // copy touch points from previous frame for comparison
...
//--------------------------------------------------------------
void kinectHandler::similarityCheck() {

    for(int v = 0; v < numScreens; v++) {
        for (int i = 0; i < touchPoints[v].size(); i ++) {
            for (int j = 0; j < previousTouchPoints[v].size(); j ++) {

                glm::vec2 check(0, 0);
                check = touchPoints[v][i] - previousTouchPoints[v][j];
                float distance = glm::length2(check);
