...
void ofApp::initSynth() {
    ...
    gain.ch(0) >> gain.ch(1);      // for temporary stereo monitoring 
...
void ofApp::keyPressed(int key){
    ...
    if (key == '2')   molSystem[0].addOrganisms(PUMPER,    1);
    if (key == '3')   molSystem[0].addOrganisms(BREATHER,  1);
    if (key == '4')   molSystem[0].addOrganisms(INTESTINE, 1);
...
void ofApp::mouseReleased(int x, int y, int button){
    ...
    float probability[4] = {0.2, 0.4, 0.95, 1.0};
