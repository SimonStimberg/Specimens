...
void molecularSystem::update() {
    ...
    if(organismsToRemove != NONE) cleanUp();
...
void molecularSystem::cleanUp() {
    ...
    if(organismsToRemove == PUMPER) {
        vector<Cell *>::iterator it = pumpers.begin();
        for(; it != pumpers.end();){
            if( (*it)->isDead ){
                delete *it;
                it = pumpers.erase(it);
    ...
    if(organismsToRemove == INTESTINE) {
        vector<Intestine *>::iterator it = intestines.begin();
        for(; it != intestines.end();){
            if( (*it)->isDead ){
                delete *it;
                it = intestines.erase(it);
            }else
                ++it;
        }
    }
