...
void molecularSystem::update() {
    ...
    // ofLogNotice("pressure: " + ofToString(getSystemPressure()));
}
...
void molecularSystem::addLiquid(float x, float y) {
    ...
    // x = -worldSize.x *0.5 - abs(velX) * 100;
    // y = velY * 10;
...
void molecularSystem::addBreather(float x, float y) {
    ...
    c->audioModule >> masterBus.ch(0);
    c->audioModule.out_void() >> blackhole;
    // synthGui.out("pitchCtrl") >> c->audioModule.in_pitch();
    // c->audioModule.out("R") >> masterBus.ch(1);

    // synthPtr->addVoice(ofRandom(60, 80));
    // c->connectAudioModule(&synthPtr->voices[synthPtr->voices.size()-1]);
