...
void Cell::set(int num, int x, int y)
{
    ...
    cycleCheck = true;
    cycleCount = 0;
    maxNumCycles = (int)ofRandom(35, 40);
...
void Cell::update()
{
    ...
    if(mature && cycleCount >= maxNumCycles) die();
...
void Cell::inflate() {
	...
	if (mature == true && guiPtr->switchOscillation ) {
        ...
        for (int i = 0; i < cellMolecules.size(); i++) {
            ...
            float expansionRadius = cellMolecules.size() * guiPtr->tuneBreatherExpansionRadius;
            ...
            expansionForce *= powf(2, guiPtr->tuneBreatherExpansionForce);
