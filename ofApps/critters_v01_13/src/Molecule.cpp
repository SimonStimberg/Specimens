...
glm::vec2 Molecule::estimateNormal(glm::vec2 p) // p is point_on_surface
{
    ...
    // vec3 n = vec3(sdfScene(p+h.xyy).x - sdfScene(p-h.xyy).x,
    //               sdfScene(p+h.yxy).x - sdfScene(p-h.yxy).x,
    //               sdfScene(p+h.yyx).x - sdfScene(p-h.yyx).x);
