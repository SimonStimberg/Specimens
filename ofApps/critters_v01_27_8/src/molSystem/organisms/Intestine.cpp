...
void Intestine::set(int num, int x, int y)
{
    ...
    nextGrowth = ofGetElapsedTimeMillis() + 10000 + (guiPtr->intestineGrowInterval) + (int)(ofRandom(guiPtr->intestineGrowInterval*0.5));
    mature = false;
    isDead = false;
    ...
    choosePitch[(int)floor(ofRandom(3))] >> audioModule.in_pitch();
...
void Intestine::digest()
{
    ...
    if ( ofGetElapsedTimeMillis() >= nextDigestion && isDigesting == false && mature && !systemPtr->doNotDigest) {
