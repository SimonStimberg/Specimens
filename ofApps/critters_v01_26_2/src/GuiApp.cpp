...
void GuiApp::setup(){
    ...
    tubeSets.resize(mainPtr->numScreens);

    for (int i = 0; i < tubeSets.size(); i++) {
        tubeSets[i].gui.setName("Canvas Shape Tube " + ofToString(i));
        tubeSets[i].gui.add(tubeSets[i].tuneCanvasWidth.set("Canvas Width", 330, 200, 360));
        tubeSets[i].gui.add(tubeSets[i].tuneCanvasHeight.set("Canvas Height", 240, 200, 288));
        tubeSets[i].gui.add(tubeSets[i].tuneVerticalBow.set("Vertical Bow", 760, 600, 2000));
        tubeSets[i].gui.add(tubeSets[i].tuneHorizontalBow.set("Horizontal Bow", 740, 600, 2000));
        tubeSets[i].gui.add(tubeSets[i].tuneEdges.set("Smooth Edges", 25, 0, 60));
        tubeSets[i].gui.add(tubeSets[i].tuneXpos.set("X Position", 0.0, -50.0, 50.0));
        tubeSets[i].gui.add(tubeSets[i].tuneYpos.set("Y Position", 0.0, -50.0, 50.0));
        tubeSets[i].gui.add(tubeSets[i].tuneRotation.set("Rotation", 0.0, -15.0, 15.0));
        gui.add(tubeSets[i].gui);

        tubeSets[i].tuneCanvasWidth.addListener(this, &GuiApp::maskListener);
        tubeSets[i].tuneCanvasHeight.addListener(this, &GuiApp::maskListener);
        tubeSets[i].tuneVerticalBow.addListener(this, &GuiApp::maskListener);
        tubeSets[i].tuneHorizontalBow.addListener(this, &GuiApp::maskListener);
        tubeSets[i].tuneEdges.addListener(this, &GuiApp::maskListener);
        tubeSets[i].tuneXpos.addListener(this, &GuiApp::maskListener);
        tubeSets[i].tuneYpos.addListener(this, &GuiApp::maskListener);
