...
glm::vec2 Molecule::repulsion() {
	...
	for (int i = 0; i < systemPtr->molecules.size(); i++) {
		...
		// special case: if its a neuron dont be repulsed by any Molecule of my Neuron (also those that i'm not connected to via a bonding)
		// if(bondings.size() > 0 && neuronPtr != NULL) {		// the condition "bondings.size() > 0" has to be set as well - not sure why - otherwise the app wont run and keep stucked in a loop

		// 	if (bondings.size() > 1) {	// check whether i am the main joint in the middle 

		// 		// test if the other Molecule is a Molecule of my Neuron
		// 		for (int j = 0; j < neuronPtr->neuronMolecules.size(); j++) { 
					
		// 			Molecule * myNeuronMolecule = neuronPtr->neuronMolecules.at(j);	// causes always that the app wont run, like stuck in a loop
		// 			// Molecule * otherNeuron = neuronPtr->neuronMolecules[i];	// causes always that the app wont run, like stuck in a loop

		// 			if(other->position == myNeuronMolecule->position) {

		// 				bondingEnergy = 0.1;
		// 				break;
		// 			}
		// 		}
		// 	} else {	if i am not the main joint only restrain repulsion towards the main joint
		// 		if(other->position == neuronPtr->neuronMolecules.at(0)->position) {
		// 			// bondingEnergy = 0.1;
		// 		}
		// 	}
		// }
...
glm::vec2 Molecule::expansion() {
	...
	if (type == CELL && cellPtr->mature == true && guiPtr->switchOscillation ) {
		...
		float expansionRadius = cellPtr->cellMolecules.size() * guiPtr->tuneExpansionRadius;
		...
		if (cellPtr->subType == BREATHER) {
			...
			oscillate = ( (ofGetElapsedTimeMillis() + cellPtr->uniqueVal * 100) % (int)(2000*freqMultiplier) <= 300) ? -0.05 : 0.01;	// oscillate between 0.04 : -0.005
		}
