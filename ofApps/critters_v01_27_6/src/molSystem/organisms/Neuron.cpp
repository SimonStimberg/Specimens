...
void Neuron::set(int arms, int elements, int x, int y)
{
    ...
    isSignaling = false;
    signalCounter = 0;
    maxNumSignals = (int)ofRandom(20, 30);
    // maxNumSignals = 20;
    ...
    audioModule.init();
    100.0 >> audioModule.in_sigDuration();
...
void Neuron::connect(Molecule * myMolecule, Molecule * otherMolecule)
{
    ...
    if(connect) {
        ...
        connections.push_back(s);                   // the Neuron keeps a copy in order to draw it and to know that it has connections
        otherNeuron->connections.push_back(s);      // give a copy also to the connected Neuron so it knows too (then its drawn twice but that's ok)
...
void Neuron::signal()
{
    ...
    if ( ofGetElapsedTimeMillis() >= nextSignal && isSignaling == false) {
        ...
        if (lastElement->bondings.size() == 2) {
            ...
            float signalDuration = guiPtr->nronSignalSpeed * signalFlow.size();
    ...
    if (isSignaling) {
        ...
        int signalSpeed = guiPtr->nronSignalSpeed; // in milliseconds the signal needs for crossing one segment
        ...
        if (segmentIdx < signalFlow.size() - 1) {
            ...
            isSignaling = false;
            // nextSignal = ofGetElapsedTimeMillis() + (guiPtr->neuronSignalInterval) + (int)(ofRandom(guiPtr->neuronSignalInterval*2));    // choose the next signal timestamp 
            int intervalTime = (int)ofMap(getSignalStatusSquared(), 0., 1., guiPtr->neuronSignalInterval, 0.f);
            nextSignal = ofGetElapsedTimeMillis() + intervalTime + (int)ofRandom(intervalTime * 2);
            signalFlow.clear();
            signalCounter++;
        }
