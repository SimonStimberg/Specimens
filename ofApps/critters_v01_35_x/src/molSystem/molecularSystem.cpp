...
void molecularSystem::addInitialDrop(int vesselType) {
    ...
    for(unsigned int i = 0; i < amount; i++){ 
        ...
        if (vesselType == 0) {
            ...
            // 672px
            organsims[0] = floor(ofRandom(22., 30.));
            organsims[1] = floor(ofRandom(1., 3.));
...
vector<Molecule *> molecularSystem::getNeighbors(float x, float y, float radius) {
	...
	std::back_insert_iterator< vector<Molecule *> > back = back_inserter(region);		// back_inserter method adapted from McCormicks approach
