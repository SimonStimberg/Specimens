...
void ofApp::setup() {
    ...
    for (int i = 0; i < numScreens; i++) {
        ...
        molSystem[i].masterBus.ch(0) >> gain.ch(0);      // channel1: breathers
            molSystem[i].masterBus.ch(0) >> sendChorusBreathers.ch(0);
            molSystem[i].masterBus.ch(0) >> sendDelayBreathers.ch(0);
        molSystem[i].masterBus.ch(1) >> gain.ch(0);       // channel2: pumpers
            molSystem[i].masterBus.ch(1) >> sendChorusPumpers.ch(0);
            molSystem[i].masterBus.ch(1) >> sendDelayPumpers.ch(0);
        molSystem[i].masterBus.ch(2) >> gain.ch(0);      // channel3: neurons
            molSystem[i].masterBus.ch(2) >> sendChorusNeurons.ch(0);
            molSystem[i].masterBus.ch(2) >> sendDelayNeurons.ch(0);
        molSystem[i].masterBus.ch(3) >> gain.ch(0);       // channel4: intestines
            molSystem[i].masterBus.ch(3) >> sendChorusIntestines.ch(0);
            molSystem[i].masterBus.ch(3) >> sendDelayIntestines.ch(0);
...
void ofApp::initSynth() {
    ...
    sendChorusBreathers.ch(0) >> chorus.ch(0);
    sendChorusPumpers.ch(0) >> chorus.ch(0);
    sendChorusNeurons.ch(0) >> chorus.ch(0);
    sendChorusIntestines.ch(0) >> chorus.ch(0);
    sendDelayBreathers.ch(0) >> delay.ch(0);
    sendDelayPumpers.ch(0) >> delay.ch(0);
    sendDelayNeurons.ch(0) >> delay.ch(0);
    sendDelayIntestines.ch(0) >> delay.ch(0);

    chorus.ch(0) >> gain.ch(0);
    delay.ch(0)  >> gain.ch(0);
    ...
    engine.listDevices();
    engine.setDeviceID(6); // REMEMBER TO SET THIS AT THE RIGHT INDEX!!!!
