...
void Cell::update()
{
    ...
    if(mature && getInflation() > 1.5) die();
}
...
void Cell::inflate() {
	...
	if (mature == true && guiPtr->switchOscillation ) {
		...
		if (subType == cellType::BREATHER) {
			oscillate = sin( (ofGetElapsedTimef()-timeOfMaturity) * freqMultiplier) * guiPtr->tuneOscillationAmount + guiPtr->tuneOscillationAmount*0.5;	// oscillate between -0.5 and 1.5
		} else {
...
void Cell::die()
{
    ...
    for (int i = 0; i < springs.size(); i++) { 
        springs[i]->removeMe = true;
    }

    systemPtr->addNeuron(cellCenter.x, cellCenter.y);
