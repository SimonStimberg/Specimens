...
void ofApp::setup() {
    ...
    molSystem.addOrganisms(LIQUID,    300);
    molSystem.addOrganisms(BREATHER,  5);
    // molSystem.addOrganisms(PUMPER,    6);
    // molSystem.addOrganisms(NEURON,    30);
    // molSystem.addOrganisms(INTESTINE, 1);
...
void ofApp::keyPressed(int key){
	if( key == ' ' ){
        ...
        // initSynth();
        molSystem.addOrganisms(BREATHER, 1);
        int allPitches[7] = {60, 63, 67, 70, 74, 80, 84};
