...
void Cell::set(int num, int x, int y)
{
    ...
    myVoice.setup(floor(ofRandom(60, 72)));
    amp_ctrl >> myVoice.in_gain();
    amp_ctrl.enableSmoothing(50.0f);
    
}


//------------------------------------------------------------------
void Cell::connectAudioModule(Voice * v)
{ 

    // voicePtr = v; 
    // amp_ctrl >> voicePtr->in_gain();
    // amp_ctrl.enableSmoothing(50.0f);
...
void Cell::update()
{
    ...
    // pdsp::ValueControl  amp;
    // amp.set(0.0);
    float newLevel = ofMap(getInflation(), 0., 1., -48.0, 0.0, true);
 
    amp_ctrl.set(dB(newLevel));
    // amp_ctrl.set(getInflation());
