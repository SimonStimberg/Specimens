...
void Neuron::signal()
{
    ...
    if (isSignaling) {
        ...
        int signalSpeed = 100; // in milliseconds the signal needs for crossing one segment
        int timeIdx = ofGetElapsedTimeMillis()-startTime;
        int segmentIdx = floor(timeIdx / signalSpeed);
        ...
        if (segmentIdx < signalFlow.size() - 1) {
            ...
            float prop = ofMap( (timeIdx) % signalSpeed, 0, signalSpeed, 0.0f, 1.0f);
