...
void Intestine::digest()
{
    ...
    if (isDigesting) {
        ...
        if (segmentIdx < intestineMolecules.size() - 2) {
            ...
            for (int i = 0; i < intestineMolecules.size(); i+=2) {

                // Molecule * other = intestineMolecules.at(i);
                glm::vec2 midPos = (intestineMolecules[i]->position + intestineMolecules[i+1]->position) * 0.5;

                glm::vec2 dist = midPos - pusher;
                float distance = abs(glm::length2(dist));    // using length2 for better performance (-> squared -> avoiding the square root operation) 



                float threshold = 10.0;
                threshold *= threshold;     // as we use length2 we have to square the radius as well
                ...
                if( distance > 0 && distance < threshold ){	// check also for > 0 because if its 0 then its checking against itself (because we do not exclude itself from the moleculeSystem pointer)
                    ...
                    float factor = ofMap(distance, 0.0, threshold, 1.0, 0.0, true);	// inverse the force -> the closer the molecules are together the stronger the repulsion

                    // factor = 1 - pow(factor,2);  // to use an exponential curve (note: the previous mapping has to be inversed though)


                    glm::vec2 stretchFrc = intestineMolecules[i]->position - intestineMolecules[i+1]->position;

                    // ofLogNotice("midPos vs stretchFrc " + ofToString(glm::normalize(midPos)) + " vs "  + ofToString(glm::normalize(stretchFrc)) );

                    stretchFrc = glm::normalize(stretchFrc) * 0.75;

                    intestineMolecules[i]  ->addForce( stretchFrc * (factor));
                    intestineMolecules[i+1]->addForce(-stretchFrc * (factor));
                    // newForce *= 2.;
