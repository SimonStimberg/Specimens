...
void molecularSystem::setup(int width, int height) {
    ...
    breatherAudioModules.resize(50);
    bAudioModInUse.assign(50, false);

    for(unsigned int i = 0; i < breatherAudioModules.size(); i++) { 
        frequency = ofRandom(220., 660.);
        breatherAudioModules[i].setup(pdsp::f2p(frequency));

        breatherAudioModules[i] >> masterBus.ch(0);          // route the audio of the organism to the corresponding stem of that orgainsm type
        breatherAudioModules[i].out_void() >> blackhole; 

        1.0 >> breatherAudioModules[i].bypass();
    }
...
void molecularSystem::addBreather(float x, float y) {
    ...
    for(unsigned int i = 0; i < bAudioModInUse.size(); i++) { 
        if (bAudioModInUse[i] == false) {
            
        }
    }

    int i = 0;
    while(bAudioModInUse[i] == true) {
